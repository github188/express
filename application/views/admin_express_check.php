        ...
        $.get('user/get_order_by_id',{id:$searchBox.val()},function(data){
            if(data){
                ...
                if(data.order_state == 0){
                    state = '<td style="text-align: center" rowspan="3">等待买家付款</td>';
                }else if(data.order_state == 1){
                    state='<td style="text-align: center" rowspan="3"><a href="admin/receive?id='+data.order_id+'&state=2" style="background:#2f70df;display: inline-block;width: 80px;text-align:center;text-indent:0;color:#fff;">立即发货</a></td>';
                }else if(data.order_state == 2){
                    state='<td rowspan="3">等待买家确认收货</td>';
